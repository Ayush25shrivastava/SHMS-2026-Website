...
<body id="top">
  ...
  <main>
    ...
    <section>
      ...
      <p>Pay the fee applicable to your category and date of payment using the conference account below. You may use
        ...
        bank-offered channels in India that credit the same account. Foreign delegates may use international wire
        transfer
        (SWIFT) where applicable. Always keep the transaction reference (UTR / RRN / transaction ID) and a screenshot or
        PDF
      ...
      <div class="registration-payment-grid" role="group" aria-label="Payment details">
        <div class="registration-payment-card">
          ...
          <dl class="registration-payment-dl">
            <dt>Account name</dt>
            <dd>SNFCE MNNIT Allahabad</dd>
            <dt>Account number</dt>
            <dd><code class="registration-payment-mono">10424975574</code></dd>
            <dt>IFSC</dt>
            <dd><code class="registration-payment-mono">SBIN0002580</code></dd>
            <dt>MICR</dt>
            <dd>211002016</dd>
            <dt>Branch</dt>
            <dd>State Bank of India, MNNIT Allahabad</dd>
            <dt>SWIFT (international)</dt>
            <dd><code class="registration-payment-mono">SBININBB828</code></dd>
          </dl>
          <p class="registration-payment-note">For NEFT/RTGS/IMPS, use these details exactly as shown. In the remarks /
            narration, include <strong>SHMS2026</strong>, your name, and category (e.g. student / faculty) if the bank
            allows.</p>
        </div>
        <div class="registration-payment-card">
          <h3 class="registration-payment-card__title">UPI &amp; scan to pay</h3>
          <p class="registration-payment-upi"><strong>UPI ID:</strong> <code
              class="registration-payment-mono">10424975574@sbi</code></p>
          <p class="registration-payment-note">You can pay via BHIM, PhonePe, Google Pay, Paytm, or any UPI app that
            supports this VPA.</p>
          <?php if ($shmsRegQrExists): ?>
            <figure class="registration-payment-qr">
              <img src="<?php echo htmlspecialchars($shmsRegQrWeb, ENT_QUOTES, 'UTF-8'); ?>" width="220" height="220"
                alt="UPI scan to pay — SHMS-2026" loading="lazy" decoding="async">
              <figcaption>Scan with your UPI app</figcaption>
            </figure>
          <?php else: ?>
            <p class="registration-payment-note">Scan-to-pay image: add <code>assets/images/scanner.png</code> to the site
              folder.</p>
          <?php endif; ?>
        </div>
      </div>
      ...
      <?php if ($shmsRegWebappUrl !== ''): ?>
        <p>After you pay, use the link below to open the official registration form in a new window. Enter your details
          and upload your payment receipt (PDF or image). The
          secretariat uses this to match your payment with your registration. You will see a confirmation on that page
          when the submission succeeds.</p>
      <?php elseif ($shmsRegPhpForm): ?>
        <p>After you pay, open the registration form in the window that appears from the button below. Enter your details
          and upload your payment receipt (PDF or image). The
          secretariat uses this to match your payment with your registration. When the submission succeeds, a printable
          receipt opens (you can print it from that page).</p>
      <?php else: ?>
      ...
      <?php if ($shmsRegWebappUrl !== ''): ?>
        <div class="registration-submit-block">
          <p class="registration-payment-cta registration-payment-cta--row">
            <a class="registration-payment-cta__btn registration-payment-cta__btn--primary" id="registration-submit"
              href="<?php echo htmlspecialchars($shmsRegWebappUrl, ENT_QUOTES, 'UTF-8'); ?>" target="_blank"
              rel="noopener noreferrer"
              onclick="try{var w=window.open(this.href,'shms2026registration','width=1000,height=900,scrollbars=yes,resizable=yes');if(w){w.focus();return false;}}catch(e){}return true;">Open
              registration form — submit details &amp; receipt</a>
          </p>
          <p class="registration-payment-note">This tries to open a dedicated window; if your browser blocks it, the link
            still opens in a new tab.</p>
        </div>
        <?php if ($shmsRegPhpAlongsideWebapp): ?>
          <div class="registration-submit-block registration-submit-block--php-fallback" id="registration-php-fallback">
            ...
            <p class="registration-payment-note">If the Google form shows an error after you choose your receipt (for
              example “did not return JSON”), use the same on-site form in a pop-up window (sends to the same Google
              endpoint).</p>
            <?php
            ...
            <?php if ($shmsRegFlash !== 'ok'):
              ...
              <p class="registration-payment-cta registration-payment-cta--row">
                <a class="registration-payment-cta__btn registration-payment-cta__btn--primary"
                  href="<?php echo $shmsRegPopHref; ?>" target="_blank" rel="noopener noreferrer"
                  onclick="try{var w=window.open('<?php echo $shmsRegPopHref; ?>','shms2026regpopup','width=1000,height=920,scrollbars=yes,resizable=yes');if(w){w.focus();return false;}}catch(e){}return true;">Open
                  registration form — this website</a>
              </p>
              <p class="registration-payment-note">Opens in a separate window. If pop-ups are blocked, the link opens in a new
                tab.</p>
            <?php endif; ?>
          </div>
        <?php endif; ?>
      <?php elseif ($shmsRegPhpForm): ?>
        <div class="registration-submit-block" <?php echo ($shmsRegFlash === 'ok' || $shmsRegFlash === 'err') ? ' id="registration-submit"' : ''; ?>>
          ...
          <?php if ($shmsRegFlash !== 'ok'):
            ...
            ?>
            <p class="registration-payment-cta registration-payment-cta--row">
              <a class="registration-payment-cta__btn registration-payment-cta__btn--primary" <?php if ($shmsRegFlash === ''): ?>id="registration-submit" <?php endif; ?> href="<?php echo $shmsRegPopHref; ?>" target="_blank"
                rel="noopener noreferrer"
                onclick="try{var w=window.open('<?php echo $shmsRegPopHref; ?>','shms2026regpopup','width=1000,height=920,scrollbars=yes,resizable=yes');if(w){w.focus();return false;}}catch(e){}return true;">Open
                registration form — submit details &amp; receipt</a>
            </p>
            <p class="registration-payment-note">Opens in a separate window. If your browser blocks pop-ups, the link still
              opens in a new tab.</p>
          <?php endif; ?>
        </div>
      <?php else: ?>
        <div class="registration-submit-block" id="registration-submit">
          <p class="registration-payment-note">Online registration submission is not enabled on this site yet. If you have
            already paid, please email
            <a href="mailto:shms2026@mnnit.ac.in">shms2026@mnnit.ac.in</a> with your payment details and receipt.
          </p>
        </div>
      <?php endif; ?>

      <?php if ($shmsRegPaymentFormUrl !== ''): ?>
        <p class="registration-payment-cta"><a class="registration-payment-cta__btn"
            href="<?php echo htmlspecialchars($shmsRegPaymentFormUrl, ENT_QUOTES, 'UTF-8'); ?>" target="_blank"
            rel="noopener noreferrer">Alternative: payment confirmation form</a></p>
      <?php endif; ?>
      ...
      <p>For any queries regarding registration or payment matching, contact the conference secretariat at
        <a href="mailto:shms2026@mnnit.ac.in">shms2026@mnnit.ac.in</a>.
      </p>
    </section>
    ...
    <section id="sponsorship" class="sponsorship-block" aria-labelledby="sponsorship-heading">
      ...
      <p class="sponsorship-lead">
        SHMS‑2026 offers a focused platform for organisations that wish to support structural health monitoring
        research,
        smart materials, and AI/ML for resilient infrastructure. Packages combine visibility with direct access to
        delegates
      ...
      <div class="sponsorship-tiers" role="list">
        <article class="sponsorship-tier sponsorship-tier--platinum" role="listitem">
          ...
          <ul class="sponsorship-tier__list">
            <li>₹1,50,000 + GST</li>
            <li>Platinum Sponsorship offers the highest level of visibility and recognition throughout the conference
            </li>
        ...
        <article class="sponsorship-tier sponsorship-tier--silver" role="listitem">
          ...
          <ul class="sponsorship-tier__list">
            <li>₹50,000 + GST</li>
            <li>Silver Sponsorship provides an excellent platform for organizations to showcase their brand and connect
              with the academic and professional community.</li>
  ...
  <?php $shmsCtaVariant = 'registration';
  require dirname(__FILE__) . '/includes/cta-bar.php'; ?>
